    ...
    class Settings
    {
        ...
        public DirectoryInfo GetModDirectory(GameCompatibleVersion version = null)
        {
            ...
            switch (ModDirectoryOption)
            {
                case DirectoryOption.AppData:
                    if (version != null)
                        return new DirectoryInfo(Path.Combine(App.Instance.AppDataPath, directoryName, version.GetFactorioShortVersion().ToString()));
                    else
                        return new DirectoryInfo(Path.Combine(App.Instance.AppDataPath, directoryName));

                case DirectoryOption.ApplicationDirectory:
                    if (version != null)
                        return new DirectoryInfo(Path.Combine(App.Instance.ApplicationDirectoryPath, directoryName, version.GetFactorioShortVersion().ToString()));
                    else
                        return new DirectoryInfo(Path.Combine(App.Instance.ApplicationDirectoryPath, directoryName));

                case DirectoryOption.Custom:
                    if (version != null)
                        return new DirectoryInfo(Path.Combine(ModDirectory, version.GetFactorioShortVersion().ToString()));
                    else
