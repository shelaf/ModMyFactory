...
namespace ModMyFactory.Models
{
    ...
    sealed partial class ModFile : IComparable<ModFile>
    {
        ...
        public void Enable()
        {
            if (InfoFile.FactorioVersion.GetFactorioShortVersion() >= FactorioVersion.DisableBehaviourSwitch) return; // Do not disable on file level for Factorio 0.17 and later.
            if (Enabled) return;
        ...
        public IList<IModSetting> GetSettings(ModCollection parentCollection, IHasModSettings owner)
        {
            ...
            if (settings == null)
            {
                ...
                foreach (var mod in parentCollection.Where(m => m.FactorioVersion.GetFactorioShortVersion() == InfoFile.FactorioVersion.GetFactorioShortVersion()))
